...
# Encurta a observação metodológica sem perder o aviso importante.
text = re.sub(
    r'<p class="incc-history-note">.*?</p>',
    '<p class="incc-history-note"><strong>* Comparação histórica normalizada:</strong> os dois caminhos começam com a mesma parcela. Desempenho passado não garante reajustes futuros.</p>',
    text,
    count=1,
    flags=re.S,
)
...
.parcel-history-base>small{font-size:9px!important;line-height:1.3!important}
...
.incc-history-note{font-size:9px!important;line-height:1.35!important;margin-top:7px!important}
...
if marker not in text:
    ...
    text = re.sub(r'/\* global-readable-v1 \*/.*?(?=</style>)', css.strip() + '\n', text, count=1, flags=re.S)
...
if sw_path.exists():
    ...
    sw = re.sub(r'calculadora-ademicon-pwa-v\d+', 'calculadora-ademicon-pwa-v10', sw)
